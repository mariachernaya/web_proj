...
header("Content-Type: application/json; charset=UTF-8"); // Для AJAX меняем на JSON
session_start();

// Для AJAX не нужно перенаправлять
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
    ...
    $language = isset($_POST['language']) ? (is_array($_POST['language']) ? $_POST['language'] : [$_POST['language']]) : [];
    $bio = isset($_POST['bio']) ? $_POST['bio'] : '';
    $check = isset($_POST['check']) ? $_POST['check'] : '';

    // Валидация
    if (empty($fio)) {
        $response['errors']['fio'] = 'Это поле пустое';
        $error = true;
    } elseif (!preg_match('/^([а-яё]+-?[а-яё]+)( [а-яё]+-?[а-яё]+){1,2}$/Diu', $fio)) {
        $response['errors']['fio'] = 'Неправильный формат: Имя Фамилия (Отчество), только кириллица';
        $error = true;
    }
    ...
    if (empty($number)) {
        ...
        $response['errors']['number'] = 'Неправильный формат, должно быть 11 символов';
        $error = true;
    } elseif ($number != preg_replace('/\D/', '', $number)) {
        $response['errors']['number'] = 'Поле должно содержать только цифры';
        $error = true;
    }
        ...
        try {
            if ($log) {
                ...
                foreach ($languages as $lang) {
                    $stmt1->execute([$_SESSION['form_id'], $lang]);
                }
            } else {
