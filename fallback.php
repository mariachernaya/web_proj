...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!$error) {
        ...
        if ($log) {
            $stmt = $db->prepare("UPDATE form_data SET fio = ?, number = ?, email = ?, bio = ? WHERE user_id = ?");
            ...
            try {
                ...
                $stmt = $db->prepare("INSERT INTO form_data (user_id, fio, number, email, bio) VALUES (?, ?, ?, ?, ? )");
                $stmt->execute([$user_id, $fio, $number, $email, $bio]);
                $fid = $db->lastInsertId();
